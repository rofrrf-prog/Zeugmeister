...
// Importiert die Konstanten direkt aus deiner config.js
  ...
  return (
    <div style={{ background: COLORS.pureWhite, borderRadius: '8px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
      ...
      <div style={{ 
        ...
        position: 'sticky', 
        top: '-15px', // Gleicht das Padding der App.js aus, damit es bündig oben klebt
        ...
        {!isEditingName ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <h2 style={{ margin: 0, fontSize: '20px', color: COLORS.textDark, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selectedCadett.vorname} {selectedCadett.nachname}</h2>
      ...
      <div style={{ padding: '20px' }}>
        ...
        {SOLL_KATEGORIEN.map(kat => {
          ...
          return (
            <div key={kat} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '11px 0', borderBottom: `1px solid ${COLORS.borderLight}` }}>
              ...
              <div>
                {geliehen ? (
                  <button onClick={() => handleRueckgabe(geliehen.id, kat, geliehen.inventar_id)} style={{ background: '#FFF5F5', color: COLORS.primaryRed, border: `1px solid ${COLORS.primaryRed}`, padding: '5px 10px', borderRadius: '4px', fontSize: '11px', cursor: 'pointer' }}>
        ...
        {/* PDF / WhatsApp Button */}
        <button onClick={generateAndSharePDF} style={{ width: '100%', marginTop: '25px', background: istVollstaendigBestaetigt ? COLORS.textDark : COLORS.warningOrange, color: COLORS.pureWhite, border: 'none', padding: '12px', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer', fontSize: '14px' }}>
